define(['angular', 'NpfMobileConfig', 'invoice/js/invoice','commonModule','areaModule','messagetipsModule','dateModule','commonFuncModule'],
            ...
            .controller('choosePayInfo', ['$scope', '$http','$location', 'NpayInfo','MessagetipsUtils','DateUtil','commonUtil',
                function ($scope,$http,$location,NpayInfo,MessagetipsUtils,DateUtil,commonUtil) {
                    ...
                    $scope.fillPayType = function() {/** 更改支付类型 */
                        ...
                        $scope.getAblePaytype($scope.invoice.monthInvoice,$scope.invoice.monthMoney,/^\d{11}$/.test($scope.invoice.phone)?["01"]:["03"],"1");//月结
                        $scope.getAblePaytype($scope.invoice.payInvoice,$scope.invoice.payMoney,/^\d{11}$/.test($scope.invoice.phone)?["01"]:["03"],"0");//非月结
                    ...
                    /** 获取可用的支付类型 */
                    $scope.getAblePaytype = function(choosenList,choosenMoney,typearr,invoicetype) {
                        ...
                        for(var i=0;i<typearr.length;i++) {
                            ...
                            for(var j=0;j<mail.length;j++) {
                                if(!commonUtil.judgeEmpty(invoicetype) && mail[j]["invoice_type"] != invoicetype) continue;
